...
const EventDetailPage = () => {
  ...
  const { getEventDetail, fetchEventStats, updateStatus, isLoading } =
    useEvent();
  ...
  if (!event)
    return (
      <div className="text-center p-20 text-gray">Evento no encontrado</div>
    );

  const analysis = getEventStatusAnalysis(
    event.startDate,
    event.endDate,
    event.status,
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            icon={Layout}
            onClick={() => navigate(`/admin/torneos?eventId=${event.id}`)}
          >
            Gestionar Torneos
          </Button>
          <Button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="bg-blue-500/5 border border-blue-500/10 p-6 rounded-lg flex items-center justify-between group hover:border-blue-500/30 transition-all">
          <div className="space-y-1">
            <p className="text-[10px] font-black text-blue-500/60 uppercase tracking-widest">
              Torneos
            </p>
            <p className="text-3xl font-bold text-light">
              {event.stats?.totalTournaments || 0}
            </p>
          </div>
        ...
        <div className="bg-purple-500/5 border border-purple-500/10 p-6 rounded-lg flex items-center justify-between group hover:border-purple-500/30 transition-all">
          <div className="space-y-1">
            <p className="text-[10px] font-black text-purple-500/60 uppercase tracking-widest">
              Equipos
            </p>
            <p className="text-3xl font-bold text-light">
        ...
        <div className="bg-amber-500/5 border border-amber-500/10 p-6 rounded-lg flex items-center justify-between group hover:border-amber-500/30 transition-all sm:col-span-2 lg:col-span-1">
          <div className="space-y-1">
            ...
            <p className="text-3xl font-bold text-light">
              {event.stats?.remainingMatches || 0}
            </p>
          </div>
          ...
          <section className="bg-[#1A1A1A] border border-white/5 p-8 rounded-lg space-y-8 shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-white/5">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-[10px] font-black text-gray/40 uppercase tracking-widest mb-1">
                    Sede Principal
                  </p>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-light font-medium">
                    {DateParser.toLongDate(event.startDate)} —{" "}
                    {DateParser.toLongDate(event.endDate)}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white/2 border border-white/5 p-6 rounded-lg">
            ...
            <p className="text-xs text-gray/40 leading-relaxed">
              Este evento se encuentra bajo la gestión de{" "}
              <strong>Olimpyx</strong>.
          ...
          <div
            className={`p-6 rounded-lg border ${analysis.bg} ${analysis.border} space-y-4`}
          >
            <div className="flex items-center gap-3">
              ...
              <h5
                className={`text-[11px] font-black uppercase tracking-widest ${analysis.color}`}
              >
                {analysis.title}
              </h5>
            </div>
      ...
      <BaseModal
        ...
        title={
          event.status === "PLANNING" ? "Activar Evento" : "Volver a Planeación"
        }
        description={
          event.status === "PLANNING"
        ...
        confirmText={
          event.status === "PLANNING" ? "Activar Ahora" : "Pausar Evento"
        }
