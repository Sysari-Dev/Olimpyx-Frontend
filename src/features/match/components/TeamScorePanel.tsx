      ...
      <div className="w-full max-w-xs mt-6 flex flex-col gap-2">
        ...
        {isVoley && onSetWin && (
          <button 
            onClick={onSetWin} 
            disabled={disabled}
            className={`w-full bg-white/5 border border-white/10 ${color} text-[10px] font-black uppercase tracking-widest py-2.5 rounded-xl transition-all ${disabledClasses}`}
          >
            Ganar Set
          </button>
        )}
